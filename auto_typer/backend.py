# Note: This script requires the 'keyboard' library for global hotkey registration.
# Platform-specific requirements:
#   - Windows: Run as Administrator for system-wide hotkey registration (optional)
#   - Linux: Run with 'sudo' for system-wide hotkey registration (optional, will show warning)
#   - macOS: Requires accessibility permissions in System Preferences
# If run without adequate privileges, some hotkeys may not register system-wide.
import json
import os
import time
import keyboard
...
def type_character_safely(char: str) -> bool:
    ...
    try:
        ...
        # Try using special_keys module first as it has better symbol mapping
        if special_keys_available and type_character:
            try:
                if type_character(keyboard, char):
                    time.sleep(0.05)
                    return True
            except Exception as e:
                print(f"[backend] special_keys typing failed for '{char}': {e}")
...
def normalize_hotkey(hotkey_str: str) -> str:
    ...
    replacements: Dict[str, str] = {
        ...
        "cmd": "windows",  # Mac command key → universal "windows" name
        "command": "windows",  # Mac command key alias → universal "windows" name
...
# Hotkey trigger throttling to avoid duplicate rapid firings
_hotkey_last_trigger: Dict[str, float] = {}
_HOTKEY_TRIGGER_MIN_GAP = 0.12  # slightly shorter responsiveness
FIRST_CHAR_SAFETY_DELAY = 0.03  # delay after releasing modifiers before first char

def _force_release_modifiers() -> None:
    """Release common modifier keys to prevent them from sticking between hotkey press and first character."""
...
# --- Timing / Behavior Tunables ---
# Maximum time to wait for hotkey keys to be released before typing starts
HOTKEY_RELEASE_MAX_WAIT = 0.5  # seconds
# Extra delay after all keys released before first character
POST_RELEASE_DELAY = 0.04  # seconds
...
def register_hotkeys(snippets: SnippetsDict) -> None:
    ...
    # Check for Linux privilege warning
    if platform.system() == 'Linux' and os.geteuid() != 0:
        print("[backend] Warning: Running on Linux without elevated privileges (sudo).")
        print("[backend] Some hotkeys may not register system-wide. For best results, run with 'sudo'.")
    ...
    for name, data in snippets.items():
        ...
        try:
            ...
            def execute_with_delay(n: str = snippet_name, combo: str = '') -> None:
                # Throttle duplicate triggers (e.g., OS sends multiple events)
                now = time.time()
                last = _hotkey_last_trigger.get(combo, 0.0)
                if now - last < _HOTKEY_TRIGGER_MIN_GAP:
                    return
                _hotkey_last_trigger[combo] = now
                print(f"[backend] Hotkey detected: {combo} -> snippet '{n}'")

                # Minimal wait for physical key release (avoid stuck modifier on first char)
                start = time.time()
                while time.time() - start < 0.25:  # shorter fail-fast window
                    still_pressed = False
                    ...
                    if not still_pressed:
                        break
                    time.sleep(0.01)
                # Force release of known modifiers just in case
                _force_release_modifiers()
                time.sleep(FIRST_CHAR_SAFETY_DELAY)
                execute_snippet(n)

            def _spawn_exec(hn_key: str = hotkey_norm, sn: str = snippet_name) -> None:  # explicit name for type clarity
                Thread(target=execute_with_delay, args=(), kwargs={'n': sn, 'combo': hn_key}, daemon=True).start()
            # Prefer immediate trigger (more reliable across some DE/WM setups)
            try:
                ...
                print(f"[backend] Primary hotkey register failed for '{hotkey_norm}' immediate trigger. Retrying on release: {reg_err}")
                handle = keyboard.add_hotkey(
                    hotkey_norm,
                    _spawn_exec,
...
# Indicates an active typing session (not paused, not finished)
typing_active: bool = False
# Lock to guard state flags across threads
_typing_state_lock: Lock = Lock()

# --- Default System Hotkeys ---
# Multiple alternatives are registered because some desktop environments
# reserve combinations like Ctrl+Space (IME toggle) or Ctrl+Esc (Start Menu).
PAUSE_HOTKEY_CANDIDATES = [
    'ctrl+space',        # Original (may be intercepted by OS/IME)
    'ctrl+shift+space',  # Alternative 1
    'ctrl+alt+space'     # Alternative 2 (rarely reserved)
]
STOP_HOTKEY_CANDIDATES = [
    'ctrl+esc',          # Original (may open menu in some DEs)
    'ctrl+shift+esc',    # Alternative 1 (often Task Manager on Windows)
    'ctrl+alt+esc'       # Alternative 2 (usually free on Linux)
]

system_hotkey_handles: List[Any] = []

def setup_system_hotkeys():
    """Sets up system-wide default hotkeys for controlling typing with fallbacks."""
    global system_hotkey_handles
    # Clear previous system hotkeys if re-registering
    for handle in system_hotkey_handles:
        try:
            keyboard.remove_hotkey(handle)
        except Exception:
            pass
    system_hotkey_handles = []

    def _try_register(hotkeys: List[str], callback: Callable[[], None], label: str) -> None:
        for combo in hotkeys:
            try:
                handle = keyboard.add_hotkey(combo, callback, trigger_on_release=False)
                system_hotkey_handles.append(handle)
                print(f"[backend] Registered {label} hotkey: {combo}")
                # Keep registering remaining candidates to give the user multiple options
            except Exception as e:
                print(f"[backend] Could not register {label} hotkey '{combo}': {e}")

    _try_register(PAUSE_HOTKEY_CANDIDATES, toggle_pause_typing, "pause/resume")
    _try_register(STOP_HOTKEY_CANDIDATES, stop_typing, "stop")
    print("[backend] System control hotkeys ready. (See README to customize)")

def toggle_pause_typing() -> None:
    """Toggles the pause state of typing. (No effect if no active session)"""
    global typing_paused, typing_active
    with _typing_state_lock:
        if not typing_active:
            return
        typing_paused = not typing_paused
        new_state = 'paused' if typing_paused else 'resumed'
    print(f"[backend] Typing {new_state}.")

def stop_typing() -> None:
    """Stops typing completely."""
    global typing_stopped, typing_active, typing_paused
    with _typing_state_lock:
        typing_stopped = True
        typing_active = False
        typing_paused = False
    # Release common modifiers to ensure next hotkeys are detected
    for mod in ("shift", "ctrl", "alt"):
        try:
            keyboard.release(mod)
        except Exception:
            pass
    print("[backend] Typing stopped and modifiers released.")

def get_control_hotkeys() -> Dict[str, List[str]]:
    """Expose the list of registered control hotkeys to the UI."""
    return {
        "pause": PAUSE_HOTKEY_CANDIDATES,
        "stop": STOP_HOTKEY_CANDIDATES
    }

def get_typing_state() -> str:
    """Return current typing state: idle|typing|paused|stopped."""
    with _typing_state_lock:
        if typing_active:
            return 'paused' if typing_paused else 'typing'
        if typing_stopped:
            return 'stopped'
        return 'idle'
...
def execute_snippet(name: str) -> None:
    ...
    """
    global typing_paused, typing_stopped, typing_active
    with _typing_state_lock:
        if typing_active and not typing_stopped:
            print("[backend] Ignoring start; another snippet still active.")
            return
        typing_paused = False
    ...
    try:
        ...
        min_delay = 0.01
        print(f"[backend] Warning: Invalid min_delay for '{name}', using default 0.01")

    try:
        max_delay = float(snippet.get("max_delay", 0.05))
    except (ValueError, TypeError):
        max_delay = 0.05
        print(f"[backend] Warning: Invalid max_delay for '{name}', using default 0.05")

    try:
        backspace_prob = float(snippet.get("backspace_probability", 0.0))
    except (ValueError, TypeError):
        backspace_prob = 0.0
        print(f"[backend] Warning: Invalid backspace_probability for '{name}', using default 0.0")

    try:
        min_backspaces = int(snippet.get("min_backspaces", 1))
    except (ValueError, TypeError):
        min_backspaces = 1
        print(f"[backend] Warning: Invalid min_backspaces for '{name}', using default 1")

    try:
        max_backspaces = int(snippet.get("max_backspaces", 3))
    except (ValueError, TypeError):
        max_backspaces = 3
        print(f"[backend] Warning: Invalid max_backspaces for '{name}', using default 3")
    ...
    def type_text_worker():
        # Declare globals inside nested function (include typing_active so it resets properly)
        global typing_paused, typing_stopped, typing_active
        ...
        try:
            for char in text:
                ...
                while True:
                    with _typing_state_lock:
                        if not typing_paused or typing_stopped:
                            break
