...
try:
    from backend import (
        ...
        get_typing_state, get_control_hotkeys
    )
except ImportError:
...
class AutoTypingApp(ttk.Frame):
    def __init__(self, master: tk.Tk):
        ...
        self.register_app_hotkeys()      # Register snippet hotkeys
        threading.Thread(
            target=self._check_updates_thread,
            daemon=True
        ).start()                        # Background update check
        self.search_entry.focus_set()    # Focus search box
        self._start_state_poll()         # Begin state polling loop
            ...
            for lbl in group_fields:
                ...
                if lbl == "Text":
                    ...
                    text_frame.grid(
                        ...
                        sticky="nsew", 
                        pady=6,
                        padx=(0, 5) # Add right padding
                    )
                    
                    form_frame.rowconfigure(grid_row_index, weight=1, minsize=100)  # meaningful minsize

                    txt_scrollbar = ttk.Scrollbar(text_frame, orient="vertical")
                    ...
                    txt_scrollbar.config(command=txt_widget.yview)
    ...
    def load_snippet_data(self, name: str) -> None:
        ...
        if snippet:
            self.fields["Name"].config(state="normal")  # Make Name field editable
    ...
    # --- Typing State Polling ---
    def _start_state_poll(self) -> None:
        self._poll_state()

    def _poll_state(self) -> None:
        try:
            state = get_typing_state()
            hotkeys = get_control_hotkeys()
            pause_keys = "/".join(hotkeys.get("pause", []))
            stop_keys = "/".join(hotkeys.get("stop", []))
            prefix = {
                "idle": "Idle",
                "typing": "Typing...",
                "paused": "Paused",
            ...
            # Only overwrite status if not currently showing a transient message (keep custom messages briefly)
            current = self.status.cget("text")
            if not current.lower().startswith("typing will start in") and not current.lower().startswith("now typing"):
                self.status.config(text=f"{prefix} | Pause: {pause_keys} | Stop: {stop_keys}")
        except Exception:
            pass
        # Schedule next poll
        self.master.after(700, self._poll_state)
